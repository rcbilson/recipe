...
func TestRecents(t *testing.T) {
	...
	// set up two recipes
	assert.NilError(t, db.Insert("http://example.com", `{"title":"recipe"}`))
	assert.NilError(t, db.Insert("http://example2.com", `{"title":"recipe2"}`))

	// ask for 5, expect 2
	recents, err := db.Recents(5)
	assert.NilError(t, err)
